"""Controlled frame-12 A/B for the 5279 sensitometric toe hypothesis.

The candidate changes only the -3.4 and -3.1 logE samples. Everything else,
including RAW decode, camera LUT, random seed, grain geometry, print/scan chain,
and display completion, remains fixed.
"""
...
# Central-line readings from the 220 dpi rendering of H-1-5279t, page 3.
# End points and all non-toe samples remain the V21 baseline.
CANDIDATE_CURVES[:, 1] = np.array([0.17522, 0.60531, 0.93451], dtype=np.float32)
CANDIDATE_CURVES[:, 2] = np.array([0.19646, 0.63186, 0.96106], dtype=np.float32)
...
def compare(name: str, baseline: np.ndarray, candidate: np.ndarray) -> dict[str, float]:
    ...
    amplified = np.clip(0.5 + 12.0 * delta, 0.0, 1.0)
    save_rgb(OUTPUT / f"difference_x12_{name}.png", np.rint(amplified * 255).astype(np.uint8))
    return {
        ...
        "baseline_luma_p5": float(np.percentile(baseline_luma, 5)),
        "candidate_luma_p5": float(np.percentile(candidate_luma, 5)),
...
def main() -> None:
    ...
    metrics: dict[str, object] = {
        ...
        "exposure_stops": EXPOSURE_STOPS,
        "baseline_toe_samples": BASELINE_CURVES[:, 1:3].tolist(),
        "candidate_toe_samples": CANDIDATE_CURVES[:, 1:3].tolist(),
        "controlled_variables": "Only SENSITO_DENSITY_RGB at logE -3.4 and -3.1 differs; random seed and all downstream parameters are identical.",
        "branches": {},
    }
    for look, name in (
