"""Controlled test of the 2383 LAD Status-A aim used by the V21 print branch.

The production source is not edited.  The candidate changes only the three
printer-light exposure aims at the 18 percent negative: V21's equal 1.00
Status-A densities become Kodak H-61B's 1.09/1.06/1.03 densities for 2383.
All spectral integration, 5279 development, neutral shapers, H-61 guard,
monitor adaptation, random seed and RAW decoding remain fixed.
"""
...
BASELINE_AIM = np.array([1.00, 1.00, 1.00], dtype=np.float32)
KODAK_2383_STATUS_A_AIM = np.array([1.09, 1.06, 1.03], dtype=np.float32)
...
def set_aim(aim_rgb: np.ndarray) -> None:
    emulsion._raw_print_2383_density_from_negative = (
        lambda negative_density_rgb: raw_print_with_aim(negative_density_rgb, aim_rgb)
    )
...
def render(raw: np.ndarray, aim_rgb: np.ndarray, look: str) -> np.ndarray:
    set_aim(aim_rgb)
...
def compare(name: str, baseline: np.ndarray, candidate: np.ndarray) -> dict[str, float]:
    ...
    save_rgb(
        OUTPUT / f"difference_x16_{name}.png",
    ...
    return {
        ...
        "oklab_median_a_shift": float(np.median(candidate_lab[..., 1] - baseline_lab[..., 1])),
        "oklab_median_b_shift": float(np.median(candidate_lab[..., 2] - baseline_lab[..., 2])),
...
def lad_density_metrics(aim_rgb: np.ndarray) -> dict[str, object]:
    set_aim(aim_rgb)
    negative = emulsion.negative_total_printer_density(
        np.array([0.18, 0.18, 0.18], dtype=np.float32)
    )
    raw = emulsion._raw_print_2383_density_from_negative(negative)
    shaped = emulsion.print_2383_density_from_negative(negative)
    return {
        "effective_5279_printer_density": np.asarray(negative).astype(float).tolist(),
        "raw_2383_status_a_density": np.asarray(raw).astype(float).tolist(),
        "gray_shaped_2383_status_a_density": np.asarray(shaped).astype(float).tolist(),
        "raw_status_a_error_from_kodak_aim": np.asarray(raw - KODAK_2383_STATUS_A_AIM).astype(float).tolist(),
        "shaped_status_a_error_from_kodak_aim": np.asarray(shaped - KODAK_2383_STATUS_A_AIM).astype(float).tolist(),
        "raw_status_a_error_rms": float(np.sqrt(np.mean(np.square(raw - KODAK_2383_STATUS_A_AIM)))),
        "shaped_status_a_error_rms": float(np.sqrt(np.mean(np.square(shaped - KODAK_2383_STATUS_A_AIM)))),
    }


def patch_metrics(aim_rgb: np.ndarray) -> dict[str, object]:
    set_aim(aim_rgb)
    values = np.array(
        [
            [0.18, 0.18, 0.18],
    ...
    projection_chroma = np.linalg.norm(projection_lab[:, 1:3], axis=-1)
    ...
    return {
        ...
        "projection_scan_oklab_delta_e": np.linalg.norm(projection_lab - scan_lab, axis=-1).astype(float).tolist(),
        "projection_scan_absolute_hue_delta_degrees": hue_delta.astype(float).tolist(),
        "projection_chroma": projection_chroma.astype(float).tolist(),
        "scan_chroma": scan_chroma.astype(float).tolist(),
...
def main() -> None:
    ...
    baseline_projection = render(raw, BASELINE_AIM, "2383_projection_monitor")
    candidate_projection = render(raw, KODAK_2383_STATUS_A_AIM, "2383_projection_monitor")
    baseline_scan = render(raw, BASELINE_AIM, "cineon_bluray")
    candidate_scan = render(raw, KODAK_2383_STATUS_A_AIM, "cineon_bluray")

    result = {
        "question": "Does replacing V21's equal 1.00/1.00/1.00 2383 Status-A LAD aim with Kodak H-61B's 1.09/1.06/1.03 aim improve the documented density constraint without harming neutral/six-colour checks?",
        "input": str(INPUT),
        "decode": "12-bit ProRes RAW via AVFoundation extended-linear BT.2020 float32",
        "frame": FRAME_INDEX,
        "dimensions": [TEST_WIDTH, TEST_HEIGHT],
        "exposure_stops": EXPOSURE_STOPS,
        "baseline_status_a_aim": BASELINE_AIM.astype(float).tolist(),
        "candidate_status_a_aim": KODAK_2383_STATUS_A_AIM.astype(float).tolist(),
        "controlled_variables": "Only the three 2383 printer-light LAD exposure aims change. Full 5279 spectral transmission, 3200 K printer lamp, 2383 record sensitivities and curves, gray shapers, xenon projection, H-61 hue guard, monitor adaptation, RAW decode, emulsion formation, DIR, grain and random seed are fixed.",
        "density_gate": {
            "baseline": lad_density_metrics(BASELINE_AIM),
            "candidate": lad_density_metrics(KODAK_2383_STATUS_A_AIM),
        },
        "patch_gate": {
            "baseline": patch_metrics(BASELINE_AIM),
            "candidate": patch_metrics(KODAK_2383_STATUS_A_AIM),
            "interpretation": "The Spirit scan is an internal provisional colour reference, not a measurement of a 5279-to-2383 LAD target.",
        },
        "frame_ab": {
            "projection": compare("projection", baseline_projection, candidate_projection),
            "scan_branch_isolation": compare("scan", baseline_scan, candidate_scan),
        },
    }
