...
The current isolated 2383 sensitometric correction and its official-vector
evidence boundary are documented in
[`V53_2383_VECTOR_CHARACTERISTIC_AUDIT_2026-08-11.md`](V53_2383_VECTOR_CHARACTERISTIC_AUDIT_2026-08-11.md).
The subsequent record-sensitivity/dye audit and remaining scan-colour boundary
are documented in
[`V54_V55_2383_SPECTRAL_VECTOR_AUDIT_2026-08-11.md`](V54_V55_2383_SPECTRAL_VECTOR_AUDIT_2026-08-11.md).
V56/V57 then expose, rather than hide, the remaining projection-colour
identifiability interval; they are diagnostic experiments and are documented in
[`V56_V57_PROJECTION_COLOUR_IDENTIFIABILITY_2026-08-11.md`](V56_V57_PROJECTION_COLOUR_IDENTIFIABILITY_2026-08-11.md).

V58 corrects a more upstream coordinate error found by re-reading that
evidence: H-61B's simultaneous integral Status-A LAD triplet had been used as
three independent separation-curve densities. The nonlinear spectral inverse,
native isolation render and revised V56/V57 interpretation are documented in
[`V58_2383_INTEGRAL_LAD_COORDINATE_AUDIT_2026-08-11.md`](V58_2383_INTEGRAL_LAD_COORDINATE_AUDIT_2026-08-11.md).

V59 completes the same official 2383 graph audit. V55 had correctly traced its
C/M/Y paths but omitted the fourth `Visual Neutral` path and therefore the
processed print's wavelength-dependent base/D-min absorption. V59 derives that
nonnegative residual, recomputes the integral LAD inverse, and keeps every other
film and display boundary isolated. Evidence, equations, observer bounds and
...
V60 performs the required cross-graph reconciliation: the spectral residual's
integrated Status-A base and the independently plotted H-D minima differ by up
to about 0.028 D. V60 keeps the official spectral shape but makes zero dye map
exactly to each H-D D-min, preventing a clear record from requesting less
absorption than the processed base. The derivation and hard closure gates are in
[`V60_2383_DMIN_COORDINATE_REGISTRATION_AUDIT_2026-08-11.md`](V60_2383_DMIN_COORDINATE_REGISTRATION_AUDIT_2026-08-11.md).

V61 then returns to the 5279 side of the chain. The previous engine used three
independent Gaussian bands as a Status-M approximation even after V51 had
recovered Kodak's complete masked-negative spectra. V61 restores ISO 5-3's
actual Status-M products and jointly inverts D-min plus all three net
dye/coupler curves to nonnegative analytical amounts. Kodak's unused Midscale
Neutral path, the physical-gamut audit, the bounded EK5218 cross-stock witness
and native T020 validation are documented in
[`V61_5279_ISO_STATUS_M_JOINT_ANALYTICAL_DENSITY_AUDIT_2026-08-11.md`](V61_5279_ISO_STATUS_M_JOINT_ANALYTICAL_DENSITY_AUDIT_2026-08-11.md).

V62 re-audits the remaining 2383 interimage surrogate after V61 changed its
upstream density coordinate. The old matrix was fitted to mixed finished-look
transforms rather than same-process chemical measurements and now produces
very large hidden physical cross-coupling. V62 withdraws it from the physical
stage as an explicit unmeasured identity endpoint. It also fixes V61's reuse of
the V60 pointwise observer lattice, so mean density and microscopic grain
density are observed by one profile-identical V62 model. Evidence, staged
Jacobians, real-frame ablation and the remaining measurement boundary are in
[`V62_INTERIMAGE_STAGE_AND_OBSERVER_LATTICE_AUDIT_2026-08-11.md`](V62_INTERIMAGE_STAGE_AND_OBSERVER_LATTICE_AUDIT_2026-08-11.md).

V63 rechecks the two remaining neutral calibrations against that corrected
chain. The final projection observer still built its gray correction from equal
principal 2383 densities, an obsolete coordinate after V58 resolved Kodak's
simultaneous Status-A LAD triplet. V63 derives the table from the actual neutral
scene -> 5279 -> printer -> 2383 trajectory. It preserves V62 bit-exactly,
keeps the scan-referenced off-neutral colour authority frozen, and explicitly
records that the density-domain six-step shaper still lacks measured off-LAD
triplets. The complete evidence and ablation are in
[`V63_2383_NEUTRAL_TRAJECTORY_AND_COLOUR_AUTHORITY_AUDIT_2026-08-11.md`](V63_2383_NEUTRAL_TRAJECTORY_AND_COLOUR_AUTHORITY_AUDIT_2026-08-11.md).

V64 then re-reads Kodak's 2383 sensitometric description and audits the other
remaining neutral transform. The engine already converts each vector-traced
separated Status-A H-D response to analytical dye amount correctly; the extra
continuous density shaper was nevertheless rewriting those official curves by
up to 0.114 D without measured off-LAD neutral-wedge triplets. V64 withdraws
only that shaper, preserves V63 and all earlier profiles bit-exactly, and keeps
off-neutral projection colour frozen. The quantitative ablation is in
[`V64_2383_PUBLISHED_CURVE_DENSITY_BOUNDARY_AUDIT_2026-08-11.md`](V64_2383_PUBLISHED_CURVE_DENSITY_BOUNDARY_AUDIT_2026-08-11.md).

V65 is a research-only audit of the next boundary and intentionally does not
promote an image profile. It proves that current projection/scan similarity is
caused by the declared scan-referenced colour publication policy, not by the
two optical models naturally converging. The generic Callier prior is tiny at
delivery, the nominal scalar flare is unreachable in both active monitor
branches, and the remaining physical-versus-scan colour interval is too large
to choose without a measured projector/screen target. See
[`V65_PROJECTION_OBSERVER_OWNERSHIP_AUDIT_2026-08-11.md`](V65_PROJECTION_OBSERVER_OWNERSHIP_AUDIT_2026-08-11.md).

V66 returns to the period data-scan path and resolves a density-coordinate
category error. The archived Spirit prior blended broad optical receivers 82%
toward independent Status-M records, even though Kodak defines Cineon data
codes as printing density. V66 calibrates the already formed 5279 negative to
the active printer-light/2383-record integral above D-min before the unchanged
0.002-D/code encoder. It does not fit the colour chart or claim proprietary
Spirit spectra. The standards trace, five-endpoint ablation, chart limits and
remaining display-finish boundary are in
[`V66_CINEON_PRINTING_DENSITY_COORDINATE_AUDIT_2026-08-11.md`](V66_CINEON_PRINTING_DENSITY_COORDINATE_AUDIT_2026-08-11.md).

V67 is a research-only ownership audit and does not promote another image
profile. It proves that Cineon density data, the current open Rec.709 display
map and the lower-scale Blu-ray finish are three different contracts. Kodak's
own data-scan and best-light documents confirm that there is no universal
Cineon-to-video inverse. The parameter ablations and next DPX/view-policy
architecture are documented in
[`V67_CINEON_DISPLAY_LAYER_OWNERSHIP_AUDIT_2026-08-11.md`](V67_CINEON_DISPLAY_LAYER_OWNERSHIP_AUDIT_2026-08-11.md).

V68 implements that separation without promoting a new image profile. The same
formed negative and Spirit aperture can now be delivered as code-exact 10-bit
RGB printing-density DPX, before any Rec.709 view or Blu-ray finish. Native V66
projection and scan masters remain bit-for-bit unchanged. The expanded tests
also found and fixed a historical-profile state leak in 2383 D-min/scanner
cache restoration. See
[`V68_CINEON_DPX_AND_PROFILE_ISOLATION_2026-08-11.md`](V68_CINEON_DPX_AND_PROFILE_ISOLATION_2026-08-11.md).

V69 then audits whether the historical scan delivery can be recreated from the
DPX alone. It cannot: the V40–V66 branch also consumes a deterministic mean
image and suppresses high-frequency opponent grain relative to that hidden
input. V69 exposes two named DPX-only view policies, retains the historical
managed result under an explicit two-input contract, and quantifies that the
old branch removes about 61% of pointwise high-frequency opponent RMS on T020
while leaving median and P99 luma unchanged. See
[`V69_NAMED_CINEON_VIEW_POLICY_AND_GRAIN_OWNERSHIP_2026-08-11.md`](V69_NAMED_CINEON_VIEW_POLICY_AND_GRAIN_OWNERSHIP_2026-08-11.md).

V70 returns to the negative after reconciling V21/V29/V34/V39/V46/V50/V69. It
shows mechanically that the public per-record 48 µm RMS curves do not identify
grain colour: current negative cross-record correlation is weak and almost
entirely owned by an unmeasured population record-mix prior, while the old
hidden-mean Blu-ray delivery is the much larger monochromatic-grain operation.
No image profile is promoted. See
[`V70_5279_JOINT_COLOUR_COVARIANCE_AUDIT_2026-08-11.md`](V70_5279_JOINT_COLOUR_COVARIANCE_AUDIT_2026-08-11.md).

V71 then separates the mechanisms that can all look like “colour cross-talk”:
record sensitivity, direct population record mixing, DIR inhibition, net
dye/mask spectra and printer integration. The direct mix is the only operator
without a separately identified physical observable; it dominates the current
negative's weak covariance and lowers colour-separation gamma, while spectral
printing remains cross-channel without it. V71 is research-only and defines the
single-variable native image gate required before identity can be promoted.
See
[`V71_5279_RECORD_COUPLING_OWNERSHIP_AUDIT_2026-08-11.md`](V71_5279_RECORD_COUPLING_OWNERSHIP_AUDIT_2026-08-11.md).

V72 executes that isolated gate. It removes only the unidentified direct
speed-population record-mix prior, leaving spectral sensitivity, DIR, net
dye/mask spectra, printing integration, tone, grain scale and observers intact.
Native T020, T003 chart/tail, 48 µm and all 57 engine regression gates pass. See
[`V72_5279_EVIDENCE_MINIMAL_RECORD_FORMATION_2026-08-11.md`](V72_5279_EVIDENCE_MINIMAL_RECORD_FORMATION_2026-08-11.md).

V73 then reopens the retained DIR prior without changing image pixels. It
expands all 54 nonzero cross-record population edges, reproduces the restrained
~0.000417 D/logE separation response, and proves that period Kodak examples
support the mechanism but not the engine's dense topology or diffusion scales.
The V72 image profile therefore remains current while the missing 5279
separation/white-light wedge observable is stated explicitly. See
[`V73_5279_DIR_TOPOLOGY_IDENTIFIABILITY_AUDIT_2026-08-11.md`](V73_5279_DIR_TOPOLOGY_IDENTIFIABILITY_AUDIT_2026-08-11.md).

V74 reconciles the fast/medium/slow model with the V24, V48 and V50 grain
history. It proves analytically and with a 5760-wide stochastic strip that the
published 48 µm RMS does not identify native point-sample power: the active
field carries about 5.1–7.0 times the aperture RMS at native scale. It also
records that the inherited V24 fine35 NPS was a perceptual selection, not a
5279 Wiener-spectrum fit, and that large exposure-dependent normalization—most
strongly 3.1–11.8× in blue—separates measured amplitude from hypothetical
microscopic populations. V72 pixels remain unchanged. See
[`V74_5279_POPULATION_ACTIVATION_AND_NATIVE_NPS_BOUNDARY_2026-08-11.md`](V74_5279_POPULATION_ACTIVATION_AND_NATIVE_NPS_BOUNDARY_2026-08-11.md).

V75 carries that native high-frequency reservoir through the actual V72
delivery files. It independently proves that the 5760→1920 area resize is an
exact 3×3 linear-light mean, shows that a sharp resize can retain about twice
the correct pure-grain RMS, and finds that the smaller-raster ProRes XQ encode
attenuates some high-pass structure. The declared review owns viewing scale;
neither its lossy amplitude nor an unknown QuickTime resize owns the emulsion
NPS. V72 pixels remain unchanged. See
[`V75_5279_SCALE_INTEGRATION_AND_QUICKTIME_VIEW_BOUNDARY_2026-08-11.md`](V75_5279_SCALE_INTEGRATION_AND_QUICKTIME_VIEW_BOUNDARY_2026-08-11.md).

V76 then freezes that exact integrated RGB16 frame and compares the existing
default `prores_ks` XQ, maximum-budget `prores_ks`, Apple VideoToolbox XQ and a
lossless FFV1 reference. Maximum-budget XQ cuts decoded mean linear error by
about 65–73% and restores high-pass retention to roughly 96–99% without
VideoToolbox's slight overshoot. Future explicit-engine deliveries now request
8192 bits/MB; the V72 film profile remains unchanged. See
[`V76_SCALE_INTEGRATED_PRORES_XQ_CODEC_AUDIT_2026-08-11.md`](V76_SCALE_INTEGRATED_PRORES_XQ_CODEC_AUDIT_2026-08-11.md).

V77 reopens the V40/V44 archive-pointwise projection-grain observer after the
V51–V64 density/spectral corrections. The direct formed-density candidate leaves
the deterministic image and scan unchanged, but nearly doubles medium-strength
mean-relative opponent impulses and is not promoted. The same audit shows that
2K integration retains only 43–47% of projection luma RMS but about 97% of its
already-low-passed opponent RMS, explaining part of the perceived scale split.
See
[`V77_FREQUENCY_OWNERSHIP_AND_PROJECTION_GRAIN_OBSERVER_2026-08-11.md`](V77_FREQUENCY_OWNERSHIP_AND_PROJECTION_GRAIN_OBSERVER_2026-08-11.md).

V78 then compares default XQ, maximum-budget XQ, VideoToolbox XQ and FFV1 on a
complete 1920×1440 uniform formed-minus-mean NPS. VideoToolbox best preserves
aggregate stochastic power but has roughly three times the T020 picture/colour
error; maximum-budget `prores_ks` remains the normal QuickTime delivery and
FFV1 remains the exact research control. See
[`V78_UNIFORM_GRAIN_CODEC_NPS_AUDIT_2026-08-11.md`](V78_UNIFORM_GRAIN_CODEC_NPS_AUDIT_2026-08-11.md).

V79 separates the two V40 projection colour-frequency boundaries that had been
carried forward together. Restoring either high-frequency projection-colour
path reopens sparse primary-colour events, so the current managed result remains
the release safety boundary. It is now explicitly classified as historical
defect containment rather than measured 5279/2383 projection physics. The same
audit shows why the colour texture can read coarser at 2K: exact integration
retains roughly 96% of the deliberately low-frequency opponent RMS but only
43–46% of luma RMS. See
[`V79_PROJECTION_GRAIN_POLICY_OWNERSHIP_2026-08-11.md`](V79_PROJECTION_GRAIN_POLICY_OWNERSHIP_2026-08-11.md).

V80 then holds every record's original 48 µm marginal RMS fixed while sweeping
frequency-band cross-record equicorrelation. Extreme correlation can improve
the aggregate opponent/luma ratio and restore more balanced 2K scale response,
but it changes luma amplitude and scan structure, fails across exposure,
retains severe nonlinear colour tails and breaks the finite nonnegative-density
bound. Post-formation covariance mixing is rejected; a future uncertainty model
must share bounded finite events during activation/development. See
[`V80_5279_CROSS_RECORD_COVARIANCE_BOUNDS_2026-08-11.md`](V80_5279_CROSS_RECORD_COVARIANCE_BOUNDS_2026-08-11.md).

V81 moves the uncertainty boundary before density formation and derives the
exact Bernoulli/Fréchet limit for shared finite activation events. An arbitrary
ρ=0.99 is feasible in only 13 of 180 tested population/pair/exposure cases. A
common-latent mixture can preserve every marginal and finite count exactly, but
its shared fraction and physical topology remain unidentified, so no sampler or
image is promoted. See
[`V81_SHARED_FINITE_EVENT_BERNOULLI_BOUNDS_2026-08-11.md`](V81_SHARED_FINITE_EVENT_BERNOULLI_BOUNDS_2026-08-11.md).

V82 adds the missing three-colour compatibility condition. Three pair targets
can each satisfy its Fréchet bound and form a positive-semidefinite correlation
matrix while still admitting no nonnegative eight-cell RGB Bernoulli law. Of
7,500 independent pair-alpha parameter sets, 3,462 are jointly impossible and
1,484 of those are PSD false positives. The V81 single-common-alpha family is
jointly valid everywhere tested, but remains unmeasured and is not promoted.
See
[`V82_THREE_RECORD_BERNOULLI_COMPATIBILITY_2026-08-11.md`](V82_THREE_RECORD_BERNOULLI_COMPATIBILITY_2026-08-11.md).

V83 then propagates the only jointly legal shared-event family through the
actual V72 five-class spatial model and stochastic DIR transfer. The audit also
corrects a documentation mistake: active V72 inherits post-coupling residual
calibration, not V39's pre-DIR dye-yield mode. All five alpha endpoints close
the digitized Kodak 48 µm RMS curves within about 1.08%, and direct finite-event
Monte Carlo verifies the analytic transfer; however, the same marginal curves
permit processed record correlations from approximately zero to 0.7–0.95. No
alpha is promoted. See
[`V83_SHARED_EVENT_DIR_RMS_CLOSURE_2026-08-11.md`](V83_SHARED_EVENT_DIR_RMS_CLOSURE_2026-08-11.md).

V84 performs the authorized paired visual uncertainty test on one native-scale
T020 crop. All endpoints consume the same finite-event random sequence and
share bit-identical deterministic means. Alpha 0→1 lowers observer opponent RMS
16–18% but raises luma RMS 43–54% and total visible RGB grain 23–26%; projection
and scan do not share one balance point. Alpha 1 is rejected as a default and
.25/.50 remain diagnostics only. The next evidence audit returns to the large
blue-record marginal itself. See
[`V84_SHARED_EVENT_VISUAL_UNCERTAINTY_2026-08-11.md`](V84_SHARED_EVENT_VISUAL_UNCERTAINTY_2026-08-11.md).

V85 returns to the March 2003 source rather than tuning the V84 result. It
re-renders the graph, evaluates the split red Bezier path correctly, fits all
twelve Sigma-D ticks, compares the page-4 companion H-D curves with page-3
Status-M sensitometry, and checks the active V72 channel mapping. The V50 trace,
four-log-unit exposure translation and V61 joint Status-M coordinate all pass.
The large blue marginal is therefore retained; the missing cross-record
covariance/cross-spectrum is the unresolved public-evidence boundary. V85
changes no pixels. See
[`V85_5279_GRANULARITY_MEASUREMENT_DOMAIN_2026-08-11.md`](V85_5279_GRANULARITY_MEASUREMENT_DOMAIN_2026-08-11.md).

V86 keeps those three measured marginals fixed and propagates the complete 3x3
PSD covariance envelope through scan, direct 2383 projection and the current
published projection observer. Shared events are not free chroma cleanup: they
raise luma grain while reducing opponent grain. The same audit then finds a
separate precision defect in the 29-cube joint Status-M-to-printer-density
stage. At -3 logE it differs from direct V61 spectral integration by as much as
0.013987 D, with the largest error in the red printer record; from -2.5 to 0
logE it stays below 0.000366 D. V86 changes no pixels and makes a direct/denser
spectral shadow gate the V87 priority. See
[`V86_OBSERVER_COVARIANCE_AND_SHADOW_SPECTRAL_LUT_AUDIT_2026-08-11.md`](V86_OBSERVER_COVARIANCE_AND_SHADOW_SPECTRAL_LUT_AUDIT_2026-08-11.md).

Bootstrap all versioned 2383 observer caches:
...
Add `--cineon-dpx` to write the independent printing-density exchange sequence.
Those DPX files require a named film-data viewing transform and must not be
judged as ordinary display RGB in a generic image viewer.
...
lists. A clean workstation can build and checksum-verify all required runtime
assets with:
...
The command is intentionally explicit because the exact atlas and 25,333
microbricks take substantial time and temporary disk space to construct.
...
The original internal V46 evidence-reconciliation note is recorded in
[`V46_EVIDENCE_RECONCILIATION_2026-08-11.md`](V46_EVIDENCE_RECONCILIATION_2026-08-11.md).
That historical laboratory note was research-only and did not change the V45
image. Its identifier is preserved. The later public V46 consolidates the
accepted V46–V86 findings, corrects the stochastic endpoint and exact spectral
inverse, and is documented separately in
[`V46_PUBLIC_CERTIFIED_SPECTRAL_INVERSE_RELEASE_2026-08-11.md`](V46_PUBLIC_CERTIFIED_SPECTRAL_INVERSE_RELEASE_2026-08-11.md).

The internal evidence-reconciliation note
separates stock-constrained facts from inferred model parameters, documents the
hybrid projection monitor-proof observer, and replaces the historical nominal
48-micrometre grain gate with a physical-aperture RMS audit:

```bash
PYTHONPATH=engine/src python3 engine/src/audit_v46_5279_aperture_rms.py
```

The subsequent grain-structure and microscopic-density-boundary audit is
recorded in
[`V47_V49_GRAIN_STRUCTURE_AND_DENSITY_BOUNDARY_2026-08-11.md`](V47_V49_GRAIN_STRUCTURE_AND_DENSITY_BOUNDARY_2026-08-11.md).
V50's replacement of the coarse historical graph transcription with values
recovered from Kodak's embedded vector paths is recorded in
[`V50_VECTOR_TRACED_GRANULARITY_2026-08-11.md`](V50_VECTOR_TRACED_GRANULARITY_2026-08-11.md).
The same source audit subsequently found larger transcription errors in the
5279 net-dye and minimum-density spectra; the isolated V51 correction is
recorded in
[`V51_VECTOR_TRACED_NEGATIVE_SPECTRA_2026-08-11.md`](V51_VECTOR_TRACED_NEGATIVE_SPECTRA_2026-08-11.md).
V52 separates the vector-traced portion of Kodak's Status-M characteristic
curves from the two unmeasured endpoint policies and validates the result on a
native 5.7K frame; see
[`V52_EVIDENCE_SEPARATED_CHARACTERISTIC_CURVES_2026-08-11.md`](V52_EVIDENCE_SEPARATED_CHARACTERISTIC_CURVES_2026-08-11.md).
