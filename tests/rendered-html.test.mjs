...
test("server-renders the bilingual V35 project home page", async () => {
  ...
  assert.match(currentSection, /CURRENT BASELINE[\s\S]{0,50}V35/);
  assert.doesNotMatch(currentSection, /CURRENT BASELINE[\s\S]{0,50}V34/);
  assert.match(html, /Grain is not an overlay[\s\S]{0,30}Grain is the image/);
  assert.match(html, /v35-t031-projection/);
  assert.match(html, /v35-t031-projection-live-srgb\.mp4/);
  assert.match(html, /v33-t031-camera-as-shot/);
  assert.match(html, /https:\/\/lovejzzz\.github\.io\/90sKid\/versions\/v35-t031-projection/);
...
test("server-renders the V35 archive, research and algorithm routes", async () => {
  ...
  assert.match(versions, /V4—[\s\S]{0,30}V35/);
  ...
  assert.match(algorithm, /CURRENT V35/);
