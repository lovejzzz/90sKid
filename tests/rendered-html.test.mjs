...
test("V48 publishes first-principles projection ownership witnesses", async () => {
  const manifest = JSON.parse(await readFile(new URL("../public/versions/v48-live-preview-manifest.json", import.meta.url), "utf8"));
  assert.equal(manifest.release_class, "first_principles_projection_ownership");
  assert.equal(manifest.frames, 24);
  assert.deepEqual(manifest.dimensions, [1920, 1440]);
  assert.equal(manifest.first_frame_audit.gates.v48_mean_is_direct_2383_bit_exact, true);
  assert.equal(manifest.first_frame_audit.gates.managed_projection_delta_is_preserved, true);
  assert.equal(manifest.first_frame_audit.gates.scan_branch_is_unmodified, true);
  assert.doesNotMatch(JSON.stringify(manifest), /\/Users\/tianxing/);
  for (const result of Object.values(manifest.verification)) {
    assert.equal(result.review_metadata.pix_fmt, "yuv444p12le");
    assert.equal(result.review_metadata.bits_per_raw_sample, "12");
  ...
  const html = await (await render("/versions")).text();
