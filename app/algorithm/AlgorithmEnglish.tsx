...
export function AlgorithmEnglish() {
  return (
    <main className="algorithm-page wrap">
      ...
      <section className="method-section"><div className="method-index">08B</div><div className="method-copy"><span className="section-tag">V31 · NORMAL-PROCESS CHROMA / TONE DECOUPLING</span><h2>2383 may reshape lightness without mistaking dye colour for silver</h2><p>V30 expressed chroma as C/L and then replaced L with the steeper 2383 neutral curve. Darker regions consequently lost absolute chroma; combined with full luminance texture, the result approached retained silver. V31 separates frequency after both complete observers: Period 2K supplies low-frequency OKLab a/b dye colour, 2383 retains its high-frequency opponent texture, and V30 per-pixel linear luma remains exact. Normal ECN-2/ECP-2D removes the silver image, so this is a process-boundary correction—not a saturation grade.</p><div className="equation"><span>FINAL OBSERVER BOUNDARY</span><b>ab<sub>out</sub>=G<sub>σ</sub>＊ab<sub>scan</sub>+[ab<sub>proj</sub>−G<sub>σ</sub>＊ab<sub>proj</sub>]　·　Y<sub>out</sub>=Y<sub>proj</sub></b><small>σ=0.72 px at 2K; gamut is compressed around target Y. 5279, print black, gamma and grain formation remain unchanged.</small></div></div></section>
