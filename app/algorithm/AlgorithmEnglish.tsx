...
export function AlgorithmEnglish() {
  return (
    <main className="algorithm-page wrap">
      <header className="page-header"><span className="eyebrow">METHOD · CURRENT V27</span><h1>Not a filter.<br />An image-formation chain.</h1><p>V27 exposes the model’s operational structure from Panasonic RAW through 5279, a 2383 print or period 2K scan, and standardized Rec.709 delivery. Creative grading remains outside the baseline.</p></header>
      ...
      <section className="method-section"><div className="method-index">01</div><div className="method-copy"><span className="section-tag">EXPOSURE</span><h2>RAW becomes three overlapping film records</h2><p>Apple’s ProRes RAW decode remains extended-linear. Panasonic’s official camera transform maps the decoded signal into V-Gamut; virtual exposure then projects it into overlapping red-, green- and blue-sensitive 5279 records.</p><div className="equation"><span>RECORD EXPOSURE</span><b>E<sub>c</sub>(x,y) = Σ<sub>j</sub>M<sub>cj</sub>RGB<sub>j</sub>(x,y)</b></div><div className="equation"><span>NEGATIVE DENSITY</span><b>D<sub>c</sub> = H<sub>c</sub>(log<sub>10</sub>E<sub>c</sub>)</b><small>Each Hc samples the published 5279 Status-M characteristic curve.</small></div></div></section>
      ...
      <section className="method-section"><div className="method-index">08</div><div className="method-copy"><span className="section-tag">MONITOR ADAPTATION</span><h2>A vendor D60 target contributes relative chroma, not a tint</h2><p>Physical projection-to-monitor presentation is not unique. The model subtracts the target transform’s own neutral response at each mean Cineon level and applies only the relative OKLab a/b difference; lightness stays physical.</p></div></section>
      ...
      <section className="validation"><span className="section-tag">V27 VALIDATION</span><h2>Release gates</h2><div className="validation-grid"><div><b>Projection</b><p>Maximum drift = 0</p></div><div><b>Scan gray residual</b><p>0.0182 → 0.00236 max</p></div><div><b>Green opponent</b><p>0.0217 → 0.00242 max</p></div><div><b>Luminance</b><p>max drift &lt; 1.8×10⁻⁷</p></div><div><b>Grain / DIR</b><p>V26 unchanged</p></div><div><b>Output</b><p>5.7K · 12-bit · Rec.709 1-1-1</p></div></div></section>
    </main>
  );
}
