...
export function AlgorithmEnglish() {
  return (
    <main className="algorithm-page wrap">
      <header className="page-header"><span className="eyebrow">METHOD · V47 VISUAL RELEASE / RESEARCH CYCLE 06</span><h1>Not a filter.<br />An image-formation chain.</h1><p>V46 remains the physical 5279 branch. Current reviewable V47 adds a controlled Silver-Efex-class SHM comparator: finite-site density with multiscale populations, local heterogeneity, clusters, voids and measured thick tails. The two are kept separate so a morphology lesson does not become a fabricated 5279 measurement.</p></header>

      <ResearchStatus language="en" />

      <section className="method-section"><div className="method-index">V47</div><div className="method-copy"><span className="section-tag">CONTROLLED SHM · MULTISCALE · THICK-TAIL</span><h2>Matching grain strength is insufficient when silver-halide organization is non-Gaussian</h2><p>Three independent isotropic populations at 0.45, 0.90 and 1.80 native pixels are locally reweighted by a slow occupancy field. A second Hermite population creates asymmetric clusters and voids; a third Hermite population creates the positive thick tails measured from a controlled Nik 8 Tri-X export. The standardized field enters an N=1250 inverse-binomial CDF, so it forms density rather than becoming an additive overlay.</p><div className="equation"><span>SHM FORMATION</span><b>Z″=Z+a(Z²−1)+0.020(Z³−3Z)　→　G=F⁻¹<sub>Binomial(1250,Y)</sub>(Φ(Z″))/1250</b><small>RMS, lag-1, skew, kurtosis, local spectral variation, opponent invariance and temporal renewal are all release gates.</small></div></div></section>
