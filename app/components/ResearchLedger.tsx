...
export function ResearchLedger({ language, compact = false }: { language: Language; compact?: boolean }) {
  ...
  return (
    <section className={`audit-ledger${compact ? " is-compact" : ""}`} id={compact ? "latest-audits" : "audit-ledger"}>
      <header>
        <span className="section-tag">{zh ? `研究周期 ${currentResearchCycle} · 四条主线` : `RESEARCH CYCLE ${currentResearchCycle} · FOUR THEMATIC THREADS`}</span>
        <h2>{zh ? "研究按问题组织，不再让一句结论冒充一个胶片版本。" : "Research is organized by questions—not by making every finding look like a film version."}</h2>
        <p>{zh ? `网站当前可观看版本为${currentVisualRelease}。下一版为${nextVisualRelease}：只在出现新的测量或可验证修正后编号。当前引擎为${currentEngineCandidate}；旧V47—V86编号仍作为脚本、报告和失败实验的不可变证据ID。` : `The current reviewable release is ${currentVisualRelease}. The next release is ${nextVisualRelease}: it will be numbered only after a new measurement or verifiable correction. The current engine is ${currentEngineCandidate}. Legacy internal V47–V86 labels remain immutable experiment IDs for scripts, reports and failed experiments.`}</p>
      </header>
