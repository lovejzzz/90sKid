...
export default function Home() {
  ...
  const sourceName = ["V42", "V41", "V40", "V39", "V38", "V37", "V36", "V35", "V34", "V33"].includes(current.version) ? "T031" : current.version === "V32" ? "T007" : "T002";
    ...
    <>
      ...
      <main>
        ...
        <section className="current-section wrap">
          ...
          <div className="current-visual-layout">
            <div className={`hero-comparison ${current.camera && !current.fsd ? "has-camera" : ""}`}>
              ...
              {current.fsd && <figure><div className="image-title"><b>FSD</b><span>FINITE-SITE DENSITY CONTROL</span></div><InteractiveImage src={current.fsd.src} previewSrc={current.fsd.src.replace(/\.jpg$/, "-sm.jpg")} videoSrc={current.fsd.videoSrc} sizes="(max-width: 680px) 100vw, 42vw" alt={`${current.version} ${sourceName} FSD finite-site density control`} gallery={currentGallery} initialIndex={2} /><figcaption>{text("共享V41冻结颜色与2383观察器的独立有限密度路线", "Independent finite-density route sharing the frozen V41 colour and 2383 observer")}</figcaption></figure>}
        ...
        <section className="v21-panel wrap">
          ...
          <ol>
            <li><b>{text("密度就是图像变量", "Density is the image variable")}</b><span>{text("V42冻结V41颜色入口，让物理5279与FSD保持不同的密度形成假设；显示RGB仍没有颗粒叠加。", "V42 freezes the V41 colour input while physical 5279 and FSD retain distinct density-formation hypotheses; display RGB still has no grain overlay.")}</span></li>
          ...
          <Link href={language === "en" ? "/research#v42-engine-en" : "/research#v42-engine"} className="button">{text("阅读V42研究一致性说明", "Read the V42 conformance record")}</Link>
        </section>
